...
| 12 | **Second opinion / chat model source** | none native | chat source per session: LAIN runtime, ChatGPT.com or Gemini.google.com (no API key, user-driven login); discovered model lists, provenance on every answer, coding authority never transferred. Replaced `/external`. | src/modelsource/*, src/chatdispatch.js; tests/unit/modelsource-conformance.test.js | CONFIRMED · LAIN-UNIQUE |

## B. Where the harnesses are at par (different shapes, same capability)

| # | Capability | Claude | LAIN | Evidence | Label / verdict |
|---|---|---|---|---|---|
| 13 | **Slash commands** | large built-in set + custom | commands.js with deterministic guards; /health /token /doctor /bg /ps /note /source; completion.js guards | src/commands.js, completion.js; smoke commands-audit | CONFIRMED · PAR |
